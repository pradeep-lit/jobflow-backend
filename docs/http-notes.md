...
An API Server with no dependency on hidden user-session context of the user from a previous request. It ideally works with Authorization header, like some cryptography like jwt.
Idempotency
Idempotent means we get the same server state after doing a task multiple times as we did it once. In APIs there much idempotency HTTP methods like GET, DELETE, PUT. POST and PATCH can be non-idempotent
...
201 comes in POST ig, which tells us creation was sucessfull and when we successfully retrieved the data.
...
When API server doesnt depend on the context or session from a previous request. Works by mathematical computation.